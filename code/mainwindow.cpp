...
MainWindow::MainWindow(QWidget *parent)
    : QGraphicsView(parent), gameOver(false)
{
    ...
    for (QGraphicsItem* item : items) {
        ...
        if (cactus) {
            ...
            if (D->collidesWithItem(cactus)) {
                ...
                gameOverText->setFont(QFont("Arial", 25, QFont::Bold));
...
void MainWindow::clearScene() {
    ...
    for (QGraphicsItem* item : view->items()) {
        if (item != dynamic_cast<QGraphicsItem*>(D)) {
...
void MainWindow::resetGame()
{
    ...
    // Resetar velocidade do chão
    Ground::speed = 3;
    Ground::empty = false;

    clearScene();

    // Recriar chão inicial
    Ground* G = new Ground();
    view->addItem(G);

    D->reset();

    // Reiniciar timers com novos objeto
    // Parar e deletar os timers antigos primeiro
    if (checkGround) {
        checkGround->stop();
        delete checkGround;
    }
